...
namespace JobSyncProvider.Areas.Identity.Pages.Account.Manage
{
    public class IndexModel : PageModel
    {
        ...
        public class InputModel
        {
            ...
            public void Update_FullName()
            {
                var nameParts = new List<string>();
                if (!string.IsNullOrWhiteSpace(FirstName)) nameParts.Add(FirstName.Trim());
                if (!string.IsNullOrWhiteSpace(MiddleName)) nameParts.Add(MiddleName.Trim().Substring(0,1) + ".");
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            // Update FirstName, MiddleName, LastName if they have changed

            var applicationUser = user as ApplicationUser;

            if (Input.FirstName != applicationUser.FirstName)
            {
                applicationUser.FirstName = Input.FirstName;
            }
            if (Input.MiddleName != applicationUser.MiddleName)
            {
                applicationUser.MiddleName = Input.MiddleName;
            }
            if (Input.LastName != applicationUser.LastName)
            {
                applicationUser.LastName = Input.LastName;
            }

            Input.Update_FullName();

            if (Input.FullName != applicationUser.FullName)
            {
